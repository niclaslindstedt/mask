...
pure modules — the generic scanning and placeholder primitives, every
detector, the masking pipeline, migrations, and the search corpus. Run one
file with `npx vitest run tests/detectors_test.ts`. UI changes should keep the
boot smoke path working: `npm run build && npm run preview`, create a project,
load the sample letter, and confirm a review.

## Documentation

If your change touches user-visible behavior, update the relevant `docs/`
topic and the README Usage section. See `AGENTS.md` for the full sync table.

## Where to ask

Bugs and feature requests: [GitHub Issues](https://github.com/niclaslindstedt/mask/issues).
Questions: open an issue too — there is no chat channel. Security problems:
