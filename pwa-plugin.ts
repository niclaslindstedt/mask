...
function channelName(base: string): { name: string; short_name: string } {
  if (base === "/preview/")
    return { name: "Mask (preview)", short_name: "Mask pre" };
  if (base === "/branch/")
...
export function buildManifest(base: string): string {
  ...
  const manifest = {
    ...
    short_name,
    description:
      "A local-first PWA that masks names, addresses, phone numbers and personal identity numbers in Swedish documents before they go to an LLM, and restores them in the answer.",
    id: base,
    start_url: base,
    scope: base,
    display: "standalone",
...
// GENERATED — do not edit. Emitted by pwa-plugin.ts for the Mask PWA.
  ...
  return {
    ...
    transformIndexHtml(): HtmlTagDescriptor[] {
      return [
        ...
        {
          tag: "meta",
          attrs: { name: "apple-mobile-web-app-title", content: "Mask" },
          injectTo: "head",
        },
      ];
    },
