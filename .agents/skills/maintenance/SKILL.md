...
This is the umbrella skill for mask, mandated by §21.6 of `OSS_SPEC.md`. It does no rewriting itself — it decides which sync skills are stale, runs each one, and reports a combined summary. Use it when you do not know which specific artifact is out of date, or when several have likely drifted at once (for example, after a large merge).
...
`.agents/skills/maintenance/.last-updated` holds the git commit hash of the last successful sweep. Empty means "never run" — use the repo's initial commit as the baseline.
...
| --------------- | ------------------------------------------------------------------------------------------- | ------------------------- | --------------------------------------------------------------- |
| `sync-oss-spec` | Repo contents vs. `OSS_SPEC.md` (structural conformance, via the standalone bash validator) | all structural §§ + §21.5 | 1 — run first so every downstream skill reads the freshest spec |
| `update-docs`   | `docs/*.md` vs. source of truth                                                             | §11.1                     | 2                                                               |
| `update-readme` | `README.md` vs. current user-facing surface                                                 | §3                        | 3                                                               |

Run order matters: `sync-oss-spec` runs first so downstream skills see the current spec; `update-docs` before `update-readme` because the README links into and summarises the docs.
...
| Changed path                              | Skill to schedule |
| ----------------------------------------- | ----------------- |
| `OSS_SPEC.md`, `.github/**`, `.agents/**` | `sync-oss-spec`   |
| `src/**`, `scripts/**`, `pwa-plugin.ts`   | `update-docs`     |
| `docs/**`, `package.json`, `Makefile`     | `update-readme`   |
...
- [ ] After all skills finish, run `make fmt`, `make lint`, `make test`
