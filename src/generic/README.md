...
| [`extractText/`](extractText)                   | File → text: text-like files read directly, PDFs through a lazily-loaded `pdfjs-dist` chunk, a geometry-driven reflow, and a style pass that writes the page's headings and emphasis back out as Markdown. |
...
| [`components/PdfView.tsx`](components)          | A PDF shown as its pages: fitted to the column, zoomable, painted as pages come into view and dropped as they leave.                                                                                       |
