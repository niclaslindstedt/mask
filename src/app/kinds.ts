...
// translated labels; a custom kind (from a pattern rule) shows its own name.
...
/** Options for a kind picker: the built-ins, plus `extra` custom kinds that
 *  already exist in the data so a picker never loses a value. */
export function kindOptions(
  t: TFn,
  extra: readonly string[] = [],
): { value: string; label: string }[] {
  const custom = [...new Set(extra)].filter((k) => !isEntityKind(k));
  return [
...
/** Highlight classes per kind — the mark colour in the review preview. */
