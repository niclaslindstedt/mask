.PHONY: build test lint fmt fmt-check actionlint release clean docs website website-dev install icons dictionaries check-seo changelog bump
...
# Rebuild the name / locality dictionaries under src/app/detectors/data/ from
# their public sources (SCB name statistics, the tätort list). Downloads into
# .cache/ on first run.
dictionaries:
